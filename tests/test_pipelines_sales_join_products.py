...
from wrappers.pipelines.sales.join_products import run_join_products

SQL_PATH = r"pipelines/sales/join_products.sql"
MODULE_KEY = r"pipelines/sales/join_products"
...
def test_join_products_per_table(spark, table, caseid, path):
...
for caseid, mapping in _bundle_params():
    def _make_test(caseid, mapping):
        def test_fn(spark):
            ...
            assert res is not None
            _ = res.count()
        test_fn.__name__ = f"test_join_products_bundle_{caseid}"
        return test_fn
    globals()[f"test_join_products_bundle_{caseid}"] = _make_test(caseid, mapping)
